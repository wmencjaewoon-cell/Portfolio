...
type CostItem = { category: string; amount: number; note?: string };
...
function normalizeAfterMerged(newMerged: any[]) {
  ...
  async function saveAll(publish: boolean) {
    ...
    try {
      ...
      const totalCostNum = totalCost.trim() === "" ? (costSum > 0 ? costSum : null) : toNumberOrNull(totalCost.trim());
      ...
      const cleanedCosts = costs
        .map((c) => {
          ...
          return { post_id: pid, category, amount, unit: "만원", quantity: 1, note };
        })
    ...
    <div style={styles.page}>
      <div style={styles.header}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button disabled={saving} onClick={() => saveAll(false)} style={styles.btn}>저장</button>
          <button disabled={saving} onClick={() => saveAll(true)} style={{ ...styles.btn, background: "#111", color: "#fff" }}>
            게시
          </button>
          <button onClick={logout} style={{ ...styles.btn, borderColor: "#ccc", color: "#333" }}>로그아웃</button>
        </div>
      </div>
  ...
  <div style={styles.grid}>
    {mergedImages.map((img: any, idx: number) => {
      ...
      <Section title="시공정보">
        ...
        <Label>총비용(만원) (비워두면 상세 합계로 저장)</Label>
        <input value={totalCost} onChange={(e) => setTotalCost(e.target.value)} style={styles.input} inputMode="numeric" />
        <div style={{ fontSize: 12, color: "#666", marginTop: 6 }}>상세 합계(UI): {costSum.toLocaleString()} 만원</div>
      </Section>

      <Section title="비용 상세(만원)">
        <button onClick={addCostRow} style={styles.btn}>+ 공종 추가</button>

        <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
          {costs.map((c, idx) => (
            <div key={idx} style={styles.costCard}>
              <input style={styles.input} placeholder="공종(철거/목공/전기...)" value={c.category} onChange={(e) => updateCost(idx, { category: e.target.value })} />
              <input style={styles.input} placeholder="금액(만원)" value={String(c.amount ?? 0)} onChange={(e) => updateCost(idx, { amount: Number(e.target.value) || 0 })} inputMode="numeric" />
              <input style={styles.input} placeholder="메모(선택)" value={c.note ?? ""} onChange={(e) => updateCost(idx, { note: e.target.value })} />
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button onClick={() => router.back()} style={styles.btn}>뒤로</button>
      </div>
    </div>
  );
}
